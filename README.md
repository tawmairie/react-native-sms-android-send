...
```js
npm install react-native-sms-android-send --save
```

## Usage Example

```js
var SmsAndroid = require('react-native-sms-android-send');
...
```gradle
...
include ':react-native-sms-android-send'
project(':react-native-sms-android-send').projectDir = new File(settingsDir, '../node_modules/react-native-sms-android-send/android')
```
...
dependencies {
    ...
    compile project(':react-native-sms-android-send')
}
```

* If using RN < 0.29, register module (in android/app/src/main/java/[your-app-namespace]/MainActivity.java)
```java
import com.tawmairie.reactnativesmsandroidsend.RNSmsAndroidPackage; // <------ add import
...
``` java
// ...
import com.tawmairie.reactnativesmsandroidsend.RNSmsAndroidPackage; // <--
